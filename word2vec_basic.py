...
def top_n_similiar(word, embeddings, word_index_map, index_word_map, top_limit):
    ...
    indexed_sort = sorted(enumerate(similarities), key=lambda x: x[1], reverse=True)

    top_n_closest = [indexed_sort[x][0] for x in range(top_limit)]

    return [index_word_map[word_index] for word_index in top_n_closest]


def filter_tuples_by_stopwords(count_tuples, stopwords):
    for count_tuple in count_tuples:
        if count_tuple[0] not in stopwords:
            yield count_tuple


def output_top_n_similar(target_words):
    for count_tuple in target_words:
        word = count_tuple[0]
        print("\nTop {0} words most related to {1}:".format(top_limit, word))
        print(top_n_similiar(word, final_embeddings, dictionary, reverse_dictionary, top_limit))


top_limit = 10

word_counts = count[1:]  # we skip UNK
descending_counts = sorted(word_counts, key=lambda x: x[1], reverse=True)

top_n_words = descending_counts[:top_limit]
